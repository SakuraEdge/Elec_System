...
def get_two_sketches(request):
    ...
    return JsonResponse({
        'T4': '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp'
              ';该模块是根据当前时段的用电量和金额，推测出当前时段的分类。将表中每一行数据，每个时间点，每个时间，每个小时的数据作为一个样本，然后这个样本的特征就是用电量和缴费金额，'
              '标签就是这个时间点是高峰平段还是低谷。就这样的数据类型到一个多层的神经网络，通过四至五层的训练，在通过分类。形成三类分类。训练完成后利用测试集对模型性能进行评估。',
        'T5': '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp'
              ';该模块是对当前用电量进行聚类分析，不仅利用了用电量这一维度，还利用了欠费信息，当前时段的状态（高峰/平段/低谷），这一离散特征。利用这三个特征作为本次聚类的特征，同时利用KMeans'
